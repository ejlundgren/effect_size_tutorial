...
Because $r_{pb}$ (point biserial correlation) is for naturally discrete groups and this can be calculated from means±sd and then you can convert to Zr (though Wolfgang and Jacobs say you shouldn't? because sampling variance isn't correct?). 
...
:::callout-important
Instead of converting SMD to Zr, it may be preferable to calculate a *biserial* correlation coefficient directly from the same summary statistics used for SMD, and convert this ($r_b$) to Zr. This can be done in `escalc` directly:
