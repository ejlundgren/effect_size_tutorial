...
There are techniques—with their own assumptions-for reconstructing the underlying data in these cases by drawing from the data generating process. See examples for [SMD-type effect sizes](3.2-smd-challenges.qmd#sec-smd-reconstructing-data), [lnOR effect sizes](3.3-lnOR-challenges.qmd#sec-lnOR-reconstructing-data), [Zr effect sizes](3.4-Zr-challenges.qmd#sec-single-axis-error-bars), and for [non-Gaussian responses](6-simulating-non-gaussian-responses.qmd),
:::

## Measurement scale {#sec-measurement-scale}

Another issue that haunts vegetation meta-analyses is plot size (**REF to that Ecology Letters paper**). Studies with larger plots (e.g., 100 m^2^) will have a smaller sample size than studies with small plots (e.g., 10 m^2^) for an equal effort. Likewise, some studies will not use areal plots at all but will instead use transects.
