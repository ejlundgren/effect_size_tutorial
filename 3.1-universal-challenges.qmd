...
Another option would be to calculate single-group effect sizes for each temperature treatment, and to use the temperature directly in our models (instead of using difference in temperature). There are a variety of single-group effect sizes which we describe in the appendix, [here](5-less-common-effect-sizes.qmd#sec-single-group).

Finally, a third option would have been to use a totally different (and more unusual effect size): **Q~10~**. This effect size would have allowed us to capture the temperature difference explicitly, thus yielding simpler models and less overall heterogeneity. We discuss this further [here](1-effect-size-overview.qmd#sec-custom-effect-sizes).
