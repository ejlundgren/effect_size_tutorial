...
We'll start with the most commonly used effect size category: effect sizes used to describe continuous responses to two discrete groups.

**Perhaps this page should be split into multiple pages**
...
There are some important differences between SMD, lnRoM, Zmu, which we summarize here. Don't stress about understanding all of these topics yet, we'll go through each one and can hopefully provide practical guidance on choosing the appropriate effect size for your meta-analysis.
...
The lnRoM point estimate is robust to the standard deviation of the data and thus more robust to overdispersion. Bias corrected lnRoM point estimate formulas are actually more biased, even at lower sample sizes and are thus not widely used

However, there is a more pernicious and underappreciated problem with lnRoM-the point estimate doubles for every added dimension in the underlying data (see @sec below where we simulate this). 

For that reason, Nakagawa (ref) recently proposed the $Z_{mu}$ statistic, which is dimensionfree and assumes a lognormal reality. Zmu converts the observed summary statistics ($m$ and $sd$) to their lognormal flavors. The math to do so is quite long, so we'll spare you the details. However, once $sd_{ln}$ and $m_{ln}$ have been calculated for each group, Zmu is calculated as follows:
...
**Does heteroscedasticity matter for lnRoM or Zmu?**
