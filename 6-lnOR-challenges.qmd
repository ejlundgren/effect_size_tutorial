...
Sometimes discrete outcomes are summarized into mean±error across trials/sites/islands/etc. This data format lends itself naturally to SMD/lnRoM/Zmu type effect sizes. However, the sample size for these effect sizes are likely to be different than for an lnOR type effect size, which uses the number of individuals as the sample size.

**FIGURE**

In these cases, you can either: 

1) convert the mean±SD to proportion and use that to calculate number of individuals

or

2) use `rbinom` to draw number of individuals? Brain is tired maybe this is stupid and I'm not thinking right. Hmmmm.
