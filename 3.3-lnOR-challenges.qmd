...
## Ordinal data {#sec-ordinal-data}

**What if a study presents several categorical outcomes?** Imagine a study presented multiple categorical outcomes instead of a binary? If your dataset is otherwise binary, you can extract this data by summing or excluding the non-relevant outcomes.

*However, if your dataset is focused entirely on a response with more than 2 discrete outcomes, we don't know what you should do.*
...
It looks like our data is not truly binomial but beta-binomial. See [here](6-simulating-non-gaussian-responses.qmd#sec-binomial) and [here](3.4-Zr-challenges.qmd#sec-zr-reconstructing-proportion-data) for how to draw from the binomial distribution.

For each our row of our extracted data (which is only 2), we will draw from the beta-binomial distribution with the R package `extraDistr` [@extradistr]. This requires the variance ($sd^2$) and two shape parameters that describe the beta distribution ($\alpha$ and $\beta$). See [here](6-simulating-non-gaussian-responses.qmd#sec-beta-binomial) for the formulas for calculating $\alpha$ and $\beta$. We'll add these to each row of the dataset `dt`:
