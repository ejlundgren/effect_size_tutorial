...
The lnRoM point estimate is thus robust to the standard deviation of the data. However, because this definition formula is still prone to bias with low sample size, bias correction again introduces issues with dispersion because the correction term includes standard deviation:

@Shinichi — but it sounds like we don't generally use lnRoM_BC because of this? Escalc defaults to the first-order formula?

$$lnRoM_{BC} = log(\frac{m1}{m2}) + 0.5 * \frac{sd_1^2}{n_1 * m_1^2}-\frac{sd_2^2}{n_2 * m_2^2}$$
Because standard deviation is in this bias-corrected estimator, this estimator can be wildly biased when data is overdispersed.
...
*NO IDEA WHICH OPTION IN ESCALC IS PAIRED. WHICH IS PRETTY EMBARRASSING!!!*
```{r}
#| echo: true
#| eval: false
