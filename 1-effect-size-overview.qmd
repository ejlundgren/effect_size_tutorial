...
However, using this definition formula produces {{< glossary bias display=biased >}} results when the sample size of those two groups is low. This is because low sample sizes lead to poor study-level estimates of $m_1$ and $m_2$. For that reason, we don't generally use the estimand or definition formula to calculate SMD. Instead we use a modified **estimator** known as Hedges' g. 
...
You'll see in future sections that certain data types should use alternative estimators, which are provided within `eff_size`. For example, if your data is normally distributed you should actually use the second-order estimator of lnRoM [@Lajeunesse2015-gw] and provided as `yi_second` and `vi_second` by `eff_size` (which are also returned by default by `escalc`). However, if your data is overdispersed, you should use the first-order estimator of lnRoM provided by `escalc` by default (and as `yi_first` and `vi_first` in `eff_size`). See [here](2.1-SMD-assumptions.qmd) where discuss this in greater depth.

These alternatives can also be accessed in `escalc` by specifying `corrected=FALSE` in the function arguments, which will return the first order definition formula estimates.
