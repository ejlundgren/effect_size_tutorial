...
The word **estimand** looks like a typo. But it isn't. An **estimand** is the target of a study: the precise thing we want to know. For example, are patients more likely to survive when given a drug, relative to a placebo group? Does selective logging lead to higher soil carbon than clearcutting? These are all estimands: the theoretical target. Primary studies and meta-analytic studies are all aiming to **estimate** an **estimand**.
...
<!-- > For example, what would today's forests would look like if there still mammoths knocking down trees? [REF] -->

<!-- 2. **The empirical estimand:** what data is actually available and what are their confounding variables?  -->

<!-- > We can't go back in time, so at best we can compare two separate populations: areas where mammoth relatives (elephants) still exist to areas where they are extinct. -->

:::callout-note
One issue with meta-analysis is that we are often forced to include studies that had different primary study-level **estimands**. In these cases, the meta-analytic model may be making an overall effect for studies that are concerned with fundamentally different things. 

This is especially true if you're including both experimental and observational (or quasi-experimental) data. Experimental data generally concerns the same target population as the control population. However, observational or quasi-experimental data are forced to compare separate populations. 

This is not to say that conducting a meta-analysis is not a worthy use of time---it is still the best way to synthesize knowledge---but that attention to study level estimands can help you be humble and honest about your meta-analytic results.
:::
...
However, this definition formula can produce {{< glossary bias display=biased >}} results when sample size is low. This is because low sample sizes lead to poor study-level estimates of $m_1$ and $m_2$. 

:::callout-note
##### Bias 
If it's been awhile since you thought about these things, imagine that your town has a true mean height, which is your estimand. However, you're unlikely to measure everyone's height. You therefore measure the height of as many individuals as you can and calculate a *sample mean* which estimates your *estimand*---the population mean. If your sample size is low (or non-random) your sample mean is likely a **biased** estimate of the true population mean height.
:::
...
Both Cohen's d and Hedges' g are estimators of the **estimand** of SMD---a mathematical description of the difference between two groups, standardized by shared standard deviation. 

**Estimands** are the mathematical target of interest, which can be estimated with a variety of **estimators** to produce your effect size **estimates**. There are sometimes multiple alternative estimators for the same estimand, which perform better or worse under different conditions. 

There are also slightly different estimands within the same effect size category that are designed for different data types---but are not comparable to other estimands. 

In general, `escalc` uses the best known estimator for each effect size type, which is often the second-order derived estimator. In the case of SMD, `escalc` uses Hedges' g. You can choose to use the first-order definition formula estimates by setting the parameter `corrected` to `FALSE`:


```{r}
#| out-width: "70%"

# The default, corrected estimate (Hedges g):
escalc(m1i = 10, m2i = 6,
       sd1i = 1.5, sd2i = 1.5,
       n1i = 15, n2i = 15,
       measure = "SMD")

# The uncorrected estimate (Cohen's d):
escalc(m1i = 10, m2i = 6,
       sd1i = 1.5, sd2i = 1.5,
       n1i = 15, n2i = 15,
       measure = "SMD",
...
::: callout-note
##### `eff_size`
Our function, `eff_this`, is similar to `escalc` but provides a little bit more control because it returns the definition formula estimates as well as derived second-order estimates if you set `default_formulas=FALSE`---and the exact formula used. This can be useful if your data does not meet certain assumptions (which we will cover in the effect size specific pages). Run the function to see the potential supported formulas and their inputs.
...
Custom effect sizes (or at least rare effect sizes) can also be useful even with data that may seem readily synthesizable with already available effect sizes.
