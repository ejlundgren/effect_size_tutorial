...
Meta-analytic effect sizes are statistics that summarize numeric distributions, and their associated variance, from empirical research. Meta-analytic effect sizes allow researchers, such as you, to synthesize data across different studies to answer big questions and to generate overall synthetic estimates.

Meta-analytic effect sizes have two parts: the **point estimate** and **sampling variance**. Point estimates are the target of interest. For example, the mean difference in cancer rates between two treatment groups. Sampling variance is the associated uncertainty around these estimates, based on sample size and/or author-reported standard deviation. Both point estimates and sampling variance are essential to fitting meta-analytic models: sampling variance is used as a weight. Effect sizes with higher sampling variance have *less* weight in meta-analytic models than effect sizes with lower sampling variance.  
...
The correlation between a continuous or discrete response variable and a continuous predictor is analyzed as the r-t-z-transformed correlation coefficient (**Zr**). This is the workhorse for meta-analyses based on observational data.
...
If you use an SMD or lnRoM-type effect size, then you should treat the *difference* in density between treatments as a moderator in your meta-analytic models. This can lead to complex models, especially if you are interested in evaluating the influence of other moderators---because each would then need an interaction to this density-difference variable.
...
In some situations, even experimental data may, potentially, be better analyzed as a single-group effect sizes if most studies used >2 experimental groups or if experimental treatments were extremely heterogeneous (e.g., different temperature treatments or different predator densities). Instead of calculating effect sizes that explicitly compare each experimental treatment to a control, you could calculate single group effect sizes for each group, and then use the various experimental groups (e.g., the temperature of each treatment) as predictors in your meta-analytic model. **Are there statistical reasons not to? E.g., inflating sample size or something?**

This applies both to continuous data summarized by mean±sd as well as binary outcome data, which can be summarized as proportions. We describe single-group effect sizes for means and proportions in the appendix, [here](5-less-common-effect-sizes.qmd#sec-single-group).
...
The dominant function used to calculate effect sizes and sampling variance in `R` is the function `escalc` in the package `metafor` [@Viechtbauer2010-uq]. `escalc` takes summary statistics, a variable specifying the effect size type (specified with the `measure` argument).
...
# Estimates, estimators, and estimands {#sec-estimates-and-estimators}

Effect size definition formulas define a theoretically *true* mathematical relationship of interest. This is called the **estimand**. For instance, the SMD point estimate estimand is defined as:
...
\end{aligned}
$$
With $m_1$ and $m_2$ as the means of two groups (e.g., a control and an experiment) and $SD_{pooled} as the pooled standard deviation of those two groups (see [here](2.1-SMD-assumptions.qmd) for the full math). This estimand defines the target mathematical relationship of SMD (and is coincidentally known as Cohen's d).

However, using this definition formula produces {{< glossary bias display=biased >}} results when sample size of those two groups is low. This is because low sample sizes lead to poor study-level estimates of $m_1$ and $m_2$. For that reason, we don't generally use the estimand formula to calculate SMD. Instead we use a modified **estimator** known as Hedges' g. 
...
\end{aligned}
$$
All of this is this is to say that **estimands** are the mathematical target of interest, which can be estimated with a variety of **estimators** to produce your effect size **estimates**. Fun stuff right?
...
Our function, `eff_this` is similar but provides a little bit more control because it returns the definition formula estimates as well as derived second-order estimates, and the exact formula used. This can be useful if your data does not meet certain assumptions (which we will cover in the effect size specific pages). Run the function to see the potential formulas support and their inputs.
...
`yi_first` and `vi_first` are the definition formulas (@eq-1 and @eq-2). the `_second` variables are the bias-corrected estimates (in this case Hedges' g). If you select `default_formulas = TRUE`, then this function will return those thought to be most stable and the least biased (e.g., the same estimates that `escalc` returns).

You'll see in future sections that certain data types should use alternative estimators, which are provided within `eff_size`. For example, if your data is normally distributed you should actually use the second-order estimator of lnRoM [@Lajeunesse2015-gw] and provided as `yi_second` and `vi_second` by `eff_size` (but not by `escalc`). However, if your data is overdispersed, you should use the first-order estimator of lnRoM provided by `escalc` by default (and as `yi_first` and `vi_first` in `eff_size`). See [here](2.1-SMD-assumptions.qmd) where discuss this in greater depth.

## Alternative estimators {#sec-alternative-estimators}

You'll also notice, for example if looking at the documentation of `escalc`, that there are many many effect size estimators based on different study designs or numeric properties of the data. For example, there are estimators for heteroscedastic data, estimators for paired study designs, for correlation data that has been discretized (e.g., into two groups, high vs low), and others of which we describe some in the [appendix](6-escalc-cheatsheet.qmd) and [here](5-less-common-effect-sizes.qmd).
...
One of the most statistically important alternative flavors are for paired designs. For example, before-after data or paired plot comparisons. These are common experimental designs in ecology and evolution but most meta-analyses do not take into account the non-independence between the control and treatment groups.
