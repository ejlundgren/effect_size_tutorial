...
dt.expanded <- dt.mlt[rep(seq_len(.N), individuals), ]
...
We have now expanded this dataset from a single row per point to a single row per individual. We can now calculate a correlation coefficient and our effect size:
...
Remember, the reason we did all of this work was to make this effect size more comparable to other effect sizes where sample size was derived from the number of individuals, which we believe would be the best way to treat data like this.

## Matrices

Some types of correlations are more complicated because they are actually based on matrix algebra. For instance, correlations between pair-wise behaviors. Imagine, that individuals engage in pairwise activities, such as grooming each other. Does grooming lead to other adaptive behaviors? Such as defending each other? This is a long-standing question in behavioral ecology.
