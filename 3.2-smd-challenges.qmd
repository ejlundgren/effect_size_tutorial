...
To reconstruct the raw binary outcomes from author reported means and sd, see our explanation [here](3.3-lnOR-challenges.qmd##sec-lnOR-reconstructing-data) and a more general tutorial about reconstructing various data types [here](7-simulating-non-gaussian-responses.qmd)
:::
...
Some studies report the ratio of the control group to the treatment group directly, without calculating a measurement of error.

For example, imagine you have paired control and experimental plots. Each of which is measured one time---e.g., percent cover. There is no measurement of error because the authors analyze the ratio or difference between the two measurements. What the hell should you do?
...
First, we reshape the data so that you can subtract the before/after treatments:
```{r}
#| echo: true
#| eval: true
...
Then subtract the means and select just the "after" standard deviation (`st_after`):
```{r}
#| echo: true
#| eval: true
...
dt.wide <- dt.wide[, sd := sd_after]
```

Now, drop the columns you don't need, cast wide again by your treatment column (`control_impact`) and calculate your effect size. 
...
Note that summarizing time series data may often create summary statistics that are *less independent* than data that may derive from separate replicates. Record these summaries for use in sensitivity analysis. See our discussion [here](3.1-universal-challenges.qmd#sec-sample-size) and @Noble2017-wv.
...
There is no way to pair any of these samples except for the final measurement for the first treatment ("Present") and the first measurement of the second treatment ("Eradicated"). This is defensible.

Alternatively, you can *draw* the underlying data based on sample size, group mean, and standard deviation extracted from the figure. This presents its own problems but is maybe better than giving up (which you should probably do. Go take a nap, sweet boy).

This should use the underlying data generating process of the response. If the response is normally distributed, or close enough, then you can use the `R` base function `rnorm`. If the data is not normally distributed then you can do a similar process, generating data from the underlying data generating process (see [here](7-simulating-non-gaussian-responses.qmd)). 

This will produce a longer dataset, with a row for every estimated sample. You'd then calculate the group mean, standard deviation, and total sample size for the binary predictor of interest, in this case "Eradicated" versus "Present". You can also do a similar process to calculate Zr, if a correlation includes error bars (see several examples [here](3.4-Zr-challenges.qmd)).
...
<details>
<summary>We've hidden a full demonstration of this here, which you can expand by clicking here</summary>
...
Voila, a dataset that constructs group means that are comparable, with an accurate sample size and pooled standard deviation. If the underlying data generating process is not normal (which is likely the case for ecological data, especially abundance data), then you probably want to use a [negative binomial](7-simulating-non-gaussian-responses.qmd#sec-negative-binomial) distribution to generate data.

The treatment variable (`status`) in this simulated dataset is *paired*, which should be reflected in your effect size estimator choice. See [here](2.1-SMD-assumptions.qmd#sec-paired-designs) for descriptions of paired estimators for SMD and lnRoM

</details>

Unfortunately, if the sample size for each of the points in the figure above is unknown, then your best option if the number of groups is ≥3, is to ignore the standard deviation and calculate a new mean from each point for the two SMD comparison groups. Or, if the number of groups is <3, to calculate Zr from just the group means per point (ignoring the standard deviation).
...
A potentially better approach would be to generate the data with `MASS:mvrnorm` and specify individual correlations within each group (`rho`), which could be set to 0.8.
:::


# Miscellaneous?
**A bit of a random selection of poop**

## Geometric mean {#sec-geometric-mean}
**@HAO, can you double check this?**


**This doesn't really belong in Sloppy reporting section**
...
## $mean$ and $sd$ of 0
@Santi --- maybe we should also have means==0 here....
...
To illustrate, we'll use a dataset provided by `escalc` and let's pretend that 3 of those studies did not report standard deviation (`sd1i`, `sd2i`):
...
**Can these be used instead of arithmetic means? According to Gemini they can, just as is. But I'm pretty sure they can't be**
