...
We here discuss effect sizes for describing continuous outcomes between two groups, such as presented by many experiments. The two dominant effect sizes in this category are Standardized Mean Differences (SMD) and the log-transformed ratio of means (lnRoM). 

:::callout-important
Note that many refer to lnRoM as lnRR (the log response ratio). However, lnRR is also the abbreviation for the log risk ratio, which we also discuss in our section about [binary](2.2-lnOR-assumptions.qmd) outcome effect sizes.
:::
...
<details>
<summary> Click here to see the spoooooky (but actually not so spooky) math </summary>
# The math
