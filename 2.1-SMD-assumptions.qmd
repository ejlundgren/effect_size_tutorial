...
As such, for both **lnRoM** and **SMD**, a negative effect size point estimate indicates that the treatment *reduced* the mean of the response variable while a positive effect size indicates that the treatment *increased* the response variable mean. 
...
# Standardized Mean Difference (SMD) {#sec-smd}
**SMD** is an effect size designed to quantify **absolute** differences between two groups, while controlling for variation. **SMD** is akin to a test statistic: it is the ratio of *signal* (the mean difference between two groups) and *noise* (the pooled standard deviation of those groups). You can see this in the point estimate formula for the definition formula of SMD (called Cohen's d):
...
$$
The formula above, for $d$, returns values between $-\infty$ and $\infty$. This values describes the number of pooled standard deviations between groups 1 and 2.
...
$$
However, Cohen's d is *biased* at low sample sizes. For that reason, we will also apply a small sample size correction factor ($J$) [@Hedges1981-bt], to calculate a bias-corrected estimator of SMD called Hedges' g. $J$ approaches 1 as sample size increases, which means that with high sample sizes, Hedges' g becomes indistinguishable from from Cohen's d:

$$
\begin{aligned}
&df = n_1+n_2-2\\
&J \approx 1-\frac{3}{4 \times df-1}\\
&g = J \times d
\end{aligned}
$$
...
&df = 20+20-2 
= 38\\
&J \approx 1-\frac{3}{4\times df-1} \approx 0.98\\
&g = Jd = 0.98 \times 0.78 = 0.76
\end{aligned}
$$

Calculating "SMD" can be done in `R` using `metafor`'s `escalc` function: 
...
### SMD Interpretation {#sec-smd-interpretation}
SMD is interpreted as number of standard deviations between the control and treatment groups. These can be further described using the following rules of thumb:
...
Note these are approximations and should be treated simply as benchmarks for interpreting your results.

Our final value of 0.76 equates to a moderately strong effect: a change in 0.76 standard deviations between the control and treatment groups.

# Log-transformed ratio of means (lnRoM)
lnRoM is intended to describe the **relative** differences between two groups---not absolute differences as in SMD. This might seem to be a trivial distinction from SMD but it has profound consequences for the results of meta-analysis (and can [even flip the direction of relationships in a metaregression](#sec-flip-signs)).

The lnRoM point estimate also only accounts for the group means---all signal and no noise. Instead, variability is captured by lnRoM's *sampling variance* estimates---which influence the *relative* weight of each effect size in your models. This can lead to very different meta-analytic results than SMD, or even than the primary studies. 

For example, if all of the primary studies in your dataset found non-significant differences between a control and experimental treatment, an SMD-based meta-analysis is likely to also find a very weak overall effect, centered on 0.

However, an lnRoM-based meta-analysis may find strong and significant differences because the **lnRoM** *point estimate* only accounts for the mean, and not for variation. 
...
**lnRoM** group means must be positive and greater than zero. It is not suitable when the means have opposite signs, which may happen when dealing with [before-after-control-impact](3.2-smd-challenges#sec-BACI) experimental designs. 
...
### lnRoM Interpretation {#sec-lnrom-interpretation}

The lnRoM point estimate of ~0.18 can be interpreted by converting it back to % change. This means that if you're fitting meta-analytic models on an lnRoM dataset, you can convert your overall model estimates and confidence intervals to % change, which can dramatically improve your ability to communicate in plain language. This is one the great advantages of lnRoM. 
...
In `R`, we can convert to % change as follows:
...
## Values near 0 {#sec-near-zero-values}
...
SMD estimates are based on the difference between $m_1$ and $m_2$. As such, an SMD calculated on means of 0.1 and 0.3 is identical to an SMD calculated on means of 10.1 and 10.3. (as long as standard deviation is the same). This is because SMD is designed for interval-scale data and describes **absolute** differences between treatment and control.
...
This is because lnRoM describes **relative** change---with multiplicative relationships between treatment and control. As such, 0s are **real** for lnRoM---true absences. 

Think carefully about whether zeroes are real in your data. In many cases they may be: an abundance measurement of 0 is a true absence. With abundance, 0 is very different from 1---while 10 is very close to 11. 

However, in other cases, this is not true and 0 is an arbitrary value on an interval scale. For example, measurements of biomass in grams can approach 0 simply because of our choice of precision (if you had measured in milligrams or nanograms then you'd still be far away from 0). Think about whether 0 is meaningful when considering whether lnRoM is the [estimand](1-effect-size-overview.qmd#sec-estimates-and-estimators) you intend to target. 

:::.callout-important
#### Conflating relative change with absolute change conflate meta-analytic conclusions and can flip results of metaregression {#sec-flip-signs}

**Also, I think this should go further up in this page**

@BECKS! Can we put the figures from your talk in here and cite those papers!?

:::

## Values ≤ 0 {#sec-zero-values}
Similarly, does your dataset have means ≤ 0? These may occur for a variety of reasons, including because of [before-after-control-impact](3.2-smd-challenges.qmd#sec-BACI) experimental designs or other data presentation formats. 

lnRoM is incalculable with means of 0: $\log(0) = -\infty$. If you add a small constant to 0s to make lnRoM calculable, you'll end up with very strong effect sizes as we [showed above](#sec-near-zero-values)---which will vary dramatically depending on what constant you use. 
...
We cover this transformation in our SMD extraction challenges page [here](3.2-smd-challenges.qmd#sec-proportion-data-with-error) 
:::
...
For lnRoM, this is a very serious problem---the estimated effect at dimension 1 will double at the second dimension and triple at the third dimension!
...
:::callout-important
The dimensionality problem of lnRoM is quite perilous given that lnRoM is one of the most commonly used effect sizes in ecology and evolution. It may be that the majority of ecological meta-analyses, especially vegetation meta-analyses, need to be redone.
:::
...
Testing for overdispersion should be part of your analytic [workflow](3.0-starting-extraction.qmd) with lnRoM and SMD type data. The easiest way to test for overdispersion is to calculate the coefficient of variation ($CV$) for your control and treatment groups. $CV$ is the ratio of standard deviation to the mean ($sd/x$).
...
An alternative approach is to use the inverse of effective sample size as your meta-analytic model weight, instead of sampling variance. This is shown in @eq-effective-n. This may be essential when a significant amount of your data is overdispersed. This is a method that produces reasonable meta-analytic estimates, despite ignoring standard deviation [@Nakagawa2023-tn].
...
For the uncorrected lnRoM, the point estimate depends only on the two means, so unequal group variances do not change that point estimate. However, the group variances do affect the sampling variance and the second-order small-sample bias corrected point estimate Heteroscedasticity therefore still matters for uncertainty and for the choice of estimator. However, this is a topic that has been little explored and is a subject for future research. **DOUBLE CHECK**
...
$$
\begin{aligned}
&d=t \times \sqrt{\frac{1}{n_1} + \frac{1}{n_2}} \\
&g = J \times d
\end{aligned}
$$
...
:::callout-note
@Yang2024-fw suggested that including both SMD and lnRoM in a bivariate model can improve model estimates. See their tutorial [here](https://yefeng0920.github.io/BRMA_tutorial_git/).
:::
