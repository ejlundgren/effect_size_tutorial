...
We start this section with effect sizes for encapsulating continuous outcomes between two discrete groups, such as presented by most experiments.
...
# Differences between SMD, lnRoM, and Zmu {#sec-smd-vs-lnrom-vs-zmu}
...
This effect size is robust to dimensionality but, like SMD and the bias-corrected lnRoM estimator, it is sensitive to overdispersion. If you were scared by the complex math we alluded to, don't worry, it's in our `eff_this` function.
...
Need to discuss what to do with when $sd = 0$ or when $m = 0$ for Zmu/lnRoM. Santi - what you wrote in your music paper was excellent. Want to paraphrase here?

## Data bounded between 0 and 1

Means±SD bounded between 0 and 1 are challenged by numeric properties: variance tends to retreat to zero when the mean approaches 0 or 1 [@Macartney2022-ik].

To resolve this for SMD, lnRoM, and Zmu effect sizes, you need to transform both the means and the standard deviations, using an arcsin-square root transformation on the mean and an equation derived using the delta method on the standard deviation (see @Macartney2022-ik). Where $m$ is the group mean and $sd$ is its standard deviation:
...
We consider a $CV>2$ to be indicative of overdispersion. However, for lnRoM\*, @Lajeunesse2015-gw suggested a small sample-size corrected threshold based on the standardized mean, derived from the Geary test (REF). Let $m$ be the group mean, $sd$ be the standard deviation and $n$ be your group sample size:
...
## ABC corrections {#sec-abc-corrections}
...
Alternative variance estimators for SMD, lnRoM, and Zmu can account for *paired* designs, that is for correlated variance between groups 1 and 2. Effect sizes derived from paired designs can be compared directly to those from unpaired designs, as long as you're using the same effect size (e.g., within the SMD or lnRoM or Zmu family).
...
For SMD, there is also a paired version that is comparable to non-paired values (e.g., you can include both in the same analysis). However, to the best of our knowledge, these are not available in `escalc`. *NO IDEA WHICH OPTION IN ESCALC IS PAIRED*
...
```
Paired SMD confusion....

```{r}
# What formula did we use?
formula <- fread("data/effect_size_formulas.csv")
unique(formula[effect_size == "SMD" & paired_design == TRUE & default == "yes", ]$formula)
