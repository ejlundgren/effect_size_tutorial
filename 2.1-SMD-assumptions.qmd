...
In this chapter, **SMD** and **lnRoM** are equally important options for comparing **mean** responses of continuous variables between two groups. 

Neither is universally better. The appropriate choice depends on the biological quantity you want to estimate and whether the data meet the assumptions of that effect size. 
...
In addition to **SMD** and **lnRoM**, continuous responses between two groups can often be complimented by analyses of different effect sizes, which capture more than just mean differences. For example, **lnM** describes magnitude of differences regardless of direction and **lnVR/lnCVR** describe differences in variability between two groups. These require the same statistical ingredients as **SMD** and **lnRoM** but are discussed in separate pages. See [here](5.2-differences-in-magnitude.qmd) 
...
# Interpretation
SMD and lnRoM have very different interpretations. These are borne from their mathematical formulations and can be summarized here:
...
$$
However, Cohen's d is *biased* at low sample sizes. For that reason, we apply a small sample size correction factor ($J$) developed by Hedges' [@Hedges1981-bt]. $J$ approaches 1 as sample size increases, which means that with high sample sizes, Hedges' g becomes indistinguishable from from Cohen's d:
...
This can all be done in `R`, using `metafor`'s `escalc` function: 
...
lnRoM can tell a very different story than SMD, which depends on the variability of your dataset. For example, if all of the studies in your dataset found non-significant differences between a control and experimental treatment, an SMD-based meta-analysis is likely to also find weak differences between those groups. 
...
With SMD, we used $J$ to correct for bias at low sample sizes. A similar second-order bias correction exists for lnRoM [@Lajeunesse2015-gw]. This correction is important **if** your dataset has small sample sizes and is **not** overdispersed (which we'll discuss further [below](#sec-overdispersion)). 
...
If your data *is* overdispersed, then this small sample size correction can increase the bias of your dataset, because the point estimate now incorporates standard deviation. In that case, you should use the uncorrected, first-order formula for lnRoM.

In `escalc`, you can do this by toggling the `correct=TRUE` to `FALSE`:
```{r}
...
```
with `_first` and `_second` referring to uncorrected definition formula and second-order, bias-corrected estimators respectively.

# Assumptions
As we describe above, SMD and lnRoM have very different interpretations. They also have different mathematical assumptions which, regardless of which interpretation you prefer, may shape your decision about which effect size to use.
...
The remaining sections examine other considerations---including negative values, dimensionality, overdispersion, heteroscedasticity, and missing standard deviations---all of which should guide your decision about whether to use SMD or lnRoM.
...
**DOES THIS APPLY TO BOTH SMD AND lnROM??**
...
### SMD paired designs
For SMD, the measure "SMCRP" [@Cousineau2020-jf] is likely safe to use in a dataset alongside unpaired "SMD" estimates, even though this formulation changes the point estimate slightly. If your data is heteroscedastic, you can use the measure "SMCRPH" [@Bonett2008-ni]. 

In `R` we can compare the results of SMD and SMCRP. Note that you only supply a single value for sample size (`ni`) instead of `n1i` and `n2i` because a paired design must by definition have equal sample sizes in each treatment. 
```{r}
#| eval: true
#| echo: true
...
```
As you can see, the point estimates differ slightly, but the sampling variance was reduced by nearly half---which means the paired estimates have considerably more influence in your models.

Our `eff_size` function provides a different paired option for SMD and SMDH that produce almost identical point estimates but also much lower sampling variance estimates:
...
rbind(unpaired %>% mutate(pairing = "unpaired"),
      paired %>% mutate(pairing = "paired"))
```
It is unclear whether SMCRP or the version in `eff_size` are better alternatives. **??????**
...
For lnRoM, both `escalc` and `eff_size` provide paired formulas: 
