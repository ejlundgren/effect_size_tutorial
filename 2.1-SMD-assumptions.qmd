...
Effect sizes are not trivial things. Together, point estimates and sampling variance are small statistical models with their own numeric assumptions and properties. Understanding these assumptions is essential if you want to conduct an accurate and robust meta-analysis and if you want to correctly interpret these effect sizes and the outputs of meta-analytic models.

We here discuss effect sizes for describing continuous outcomes between two groups, such as presented by many experiments. The two dominant effect sizes in this category are **Standardized Mean Difference** (SMD) and the **log-transformed ratio of means** (lnRoM). 

:::callout-important
Note that many refer to log-transformed ratio of means as lnRR (the log response ratio or just response ratio). However, lnRR is also the abbreviation for the *log risk ratio*, which we also discuss in our section about [binary](2.2-lnOR-assumptions.qmd) outcome effect sizes.
:::

These are among the most common class of effect sizes, especially in ecology and evolution. These effect sizes compare the mean±error of a continuous predictor measured for two groups, such as an experiment and control group. 
...
Choosing whether to use SMD or lnRoM is not simple. SMD and lnRoM have fundamentally different interpretations. The lnRoM point estimate describes the difference between means of two groups, regardless of the variance of these groups. Signal but no noise! On the other hand the SMD point estimate is akin to a test statistic: it accounts for the variance of each group and is calculated as the difference between means (the signal) relative to noise (standard deviation). 

These conceptual differences, which derive from their mathematics, have profound consequences for the types of data they can accept and for their interpretation.
...
SMD describes the difference between two group means, standardized by pooled standard deviation. The simplest estimator of SMD is Cohen's d. With $sd$, $m$, and $n$ the standard deviation, mean, and sample size of the extracted data for groups 1 and 2, Cohen's d is defined as:
...
Cohen's d can be biased in its estimate of SMD at low sample sizes. For that reason, @Hedges1981-bt did some fancy math to create a small-sample size bias correction ($J$), which approaches 0 as total $n$ increases. This formulation is known as Hedges' g and is the primary estimator of SMD.
...
The main take away from these formulas is that the point estimate (both $d$ and $g$) are influenced by standard deviation. In other words, the signal (the difference between means) is standardized by noise (the pooled standard deviation). 

Observations with high standard deviation will have a lower overall point estimate, even if the means are very different.

Because this can reduce power in your model if your data is overdispersed, @Hedges1999-al later proposed the log-transformed ratio of means, the most widely used effect size in ecology (@REF to Nakagawa/Yefeng). 
...
However, @Lajeunesse2015-gw showed that this point estimate (@eq-5) is biased at low sample sizes. For that reason, @Lajeunesse2015-gw created a "second-order" bias-corrected version of lnRoM using the Delta method.

$$
\begin{aligned}
&lnRoM = log(\frac{m1}{m2}) + \frac{1}{2}(\frac{sd_1^2}{n_1 m_1^2} - \frac{sd_2^2}{n_2 m_2^2})\\
\end{aligned}
$${#eq-6}

The correction factor approaches 0 as sample size increases and it does not ultimately affect the interpretation of lnRoM: variance is not in the denominator. lnRoM still is all signal, and not a signal to noise ratio like SMD.

Both the first-order definition formula of lnRoM (@eq-6) and @Lajeunesse2015-gw's second-order estimator are available in our `eff_size` function (under the names `yi_first` and `yi_second`. Note, you have to run the function with `default_formulas = FALSE`:
...
`yi_first` is the definition formula (@eq-1) of lnRoM and `yi_second` is the bias-adjusted estimator produced by @Lajeunesse2015-gw (and the `vi` values are the corresponding sampling variances).

Note that `escalc` returns just the first order point estimate:
...
For that reason, if your data is not overdispersed (see [below](#sec-overdispersion)) and you are using lnRoM as your effect size you may want to consider using `eff_size` and `yi_second`.

# Should I use SMD or lnRoM? {#sec-smd-or-lnrom}

SMD and lnRoM have different numeric properties that create inherently different interpretations. In a way SMD is itself a test statistic: a ratio of signal (the difference in means) to noise (standard deviation). 

On the other hand, lnRoM simply summarizes the ratio of two means---with the sampling variance of lnRoM weighting results by their *relative* sampling variance in your models. As such the same data can produce a very weak SMD effect size value, close to 0, but an extremely high lnRoM value.

We summarize these in the table below. Don't stress about understanding all of these topics yet, we'll go through each one and can hopefully provide practical guidance on choosing the appropriate effect size for your meta-analysis.
...
However, SMD values are difficult to interpret. They ultimately mean the number of standard deviations between each group but that can be difficult to convey or interpret. In general, people use the following rules of thumb to interpret SMD values:
...
An alternative to the SMD interpretation rules of thumb above is to convert SMD to the "Common Language Effect Size" (CLES) [@McGraw1992-ae]. This can be discussed in terms of the chance that a **randomly picked item from group 1 will have a higher value than a randomly picked item from group 2.**

*Is it possible to convert model estimates of SMD (Hedges' g) to CLES? If not, then let's delete this section. This is only useful if it provides a way to "plain english" model estimates*
...
**Does heteroscedasticity matter for lnRoM sampling variance or 2nd-order point estimate???? (I know it doesn't matter for first order)**
...
However, we also recommend doing analyses with both SMD and lnRoM separately as sensitivity analyses. If the results are fundamentally similar (e.g., same direction and significance), then you can choose to make lnRoM the main-text results, converted to % change. 
