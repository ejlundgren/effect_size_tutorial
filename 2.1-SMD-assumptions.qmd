...
However, @Lajeunesse2015-gw showed that this point estimate (@eq-5) is biased at low sample sizes. For that reason, @Lajeunesse2015-gw also created a "2nd-order" bias-corrected version of lnRoM using the Delta method.

**MATH?**

Both of these estimators of lnRoM are available in our `eff_size` function (under the name `yi_second` and `vi_second`. Note, you have to run the function with `default_formulas = FALSE`:
...
`yi_first` is the definition formula (@eq-1) of lnRoM and `yi_second` is the bias-adjusted estimator produced by @Lajeunesse2015-gw.
...
For that reason, if using lnRoM as your effect size you may want to consider using `eff_size` and `yi_second`, if your data is not overdispersed (see [below](#sec-overdispersion))
