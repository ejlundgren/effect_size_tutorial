...
### Specifying $rho$
The correlation between groups, $rho$ (`ri` in `escalc`) is rarely reported and so needs to be assigned based, basically, on guesswork. We'd suggest that if the pairing is within an individual or plot (e.g., before-after a treatment), then a rho of 0.8 seems reasonable. However, if the pairing is based on paired independent plots (e.g., adjacent to each other) then perhaps a rho of 0.5 would be more reasonable. 

Consider conducting sensitivity analyses on these choices, to ensure that they aren't unduly changing your model estimates.
