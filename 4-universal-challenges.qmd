...
::: callout-important
In the example above, we are forced to use the difference in temperature as a moderator. However, alternatively, we could have  calculated Zr, after drawing the underlying data for each temperature treatment with the mean, standard deviation, and sample size (see @sec). Alternatively, we could have used a totally different (and more unusual effect size): Q~10~. This effect size would have allowed us to capture the temperature difference explicitly, thus yielding simpler models and less overall heterogeneity.
